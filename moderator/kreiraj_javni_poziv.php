<?php
    ...
    if(isset($_COOKIE["pobrisi"]) && ($_COOKIE["pobrisi"] !== '-1')){
        ...
        $dnevnik_upit = "INSERT INTO `dnevnik` VALUES (NULL, 'brisanje', 'DELETE FROM javni_poziv WHERE javni_poziv_id = {$id}' , '{$vrijeme_provodenja}', '{$dnevnik_korisnik}', '2')";
    ...
    if(isset($_POST["azuriraj"]) && ($_COOKIE["id"] !== '-1')){
        ...
        $dnevnik_upit = "INSERT INTO `dnevnik` VALUES (NULL, 'ažuriranje', 'UPDATE javni_poziv SET broj_mjesta = {$broj_mjesta}, datum_od = {$datum_od}, datum_do = {$datum_do} WHERE javni_poziv_id = {$id}' , '{$vrijeme_provodenja}', '{$dnevnik_korisnik}', '2')";
        $rezultat3 = $veza->updateDB($dnevnik_upit);
    ...
    if(isset($_POST["dodaj"])){
        ...
        $dnevnik_upit = "INSERT INTO `dnevnik` VALUES (NULL, 'dodavanje', 'INSERT INTO `javni_poziv` (`javni_poziv_id`, `broj_mjesta`, `datum_od`, `datum_do`, `vrtic_id`, `korisnik_id`) VALUES (NULL, {$broj_mjesta}, {$datum_od}, {$datum_do}, {$vrtic_id_a}, {$korisnik_id_a})' , '{$vrijeme_provodenja}', '{$dnevnik_korisnik}', '2')";
        $rezultat3 = $veza->updateDB($dnevnik_upit);
...
<head>
    <title>Kreiraj javni poziv</title>
...
<header class="header">
        ...
        <div class="sadrzaj_administrator">
            ...
            echo "<div class='desni_dio'><h2>Kreiraj Javni pozivi</h2>
            <hr>";
                echo "<form action='kreiraj_javni_poziv.php' method='POST'>
                    ...
                    <tr>
                        <td><input type='number' id='broj_mjesta' name='broj_mjesta' placeholder='Broj mjesta' value='{$broj_mjesta}'></td>
                        <td><input type='date' id='datum_od' name='datum_od'  value='{$datum_od}'></td>
                        <td><input type='date' id='datum_do'  name='datum_do'  value='{$datum_do}'></td>
                    </tr>
                    <tr>
                    <td></td>
                    <td></td>
